define(['vuex', 'vue', 'jquery', 'chartsjs', 'axios'], function (Vuex, Vue, $, Chart, axios) {
        ...
        function generateStatItem(inProduction = true) {
            let clickCost = (Math.random()).toFixed(4)
            let id = inProduction ? 0 : (Math.random() * 100000).toFixed(0)
            let views = inProduction ? 0 : (Math.random() * 1000).toFixed(0)
            let clicks = inProduction ? 0 : (Math.random() * 100).toFixed(0)
            let money = inProduction ? 0 : (clicks * clickCost * (Math.random()).toFixed(2))
            let conversions = inProduction ? 0 : (Math.random() * 10).toFixed(0)
            return {
                id: id,
                views: views,
                clicks: clicks,
